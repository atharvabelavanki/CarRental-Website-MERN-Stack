...
const ManageCars = () => {
  ...
  const toggleAvailability = async (carId) => {
    try {
      ...
      if (data.success) {
        ...
        await fetchOwnerCars(); // ensure latest list is loaded
  ...
  const deleteCar = async (carId) => {
    try {
      ...
      const { data } = await axios.post('/api/owner/delete-car', { carId }); // ✅ fixed URL
      if (data.success) {
        toast.success(data.message);
        await fetchOwnerCars(); // ✅ re-fetch updated car list
        fetchCars(); // refresh global car list
      } else {
        toast.error(data.message);
      }
